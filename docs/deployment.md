...
**Security note (cluster command bus):** cluster commands (disconnect, takeover, publish, survey) are delivered to each node incarnation's Redis Stream inbox (at-least-once via a consumer group, PR-KA-C3) and their replies travel over Redis Pub/Sub; both directions are protected by an **HMAC-SHA256 hard gate** (PR-KA-B4, see `internal/cluster/hmac` and the package comment in `pkg/redisbroker/cluster_command_bus.go`). The key comes only from node configuration — `cluster.hmac_key` or `cluster.hmac_key_file` (at least 32 bytes; a key file's single trailing newline is trimmed). Startup is refused when the cluster is enabled without exactly one key source, and the bus itself rejects a short key. The key is never written to any Redis key, stream entry, publish payload, log line, or metrics label. Unsigned, badly signed, or clock-skewed (±30s) commands are rejected before claiming or handling (and still XACKed so they are not redelivered), and forged replies never count as success; rejections are counted in `messageloop_cluster_command_hmac_reject_total{reason}`. Commands still carry an `IssuedBy` audit field (sender `node_id`), which is forgeable and informational only. Redis network isolation remains recommended as defense in depth (private network, authentication/firewalling, and separate ACLs for the control-plane vs data-plane where available), but it is no longer the only boundary.
