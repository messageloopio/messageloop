...
本目录是 MessageLoop 的开发者文档套件，覆盖服务端架构、配置、管理接口、集群、可观测性、开发流程与官方 SDK。全部内容以简体中文撰写。文中所有类型名、函数名、指标名、配置字段与行为均以仓库源码为准；引用代码时只给出文件路径与符号名，不写行号（行号会随重构漂移）。
...
| [管理 API 参考](03-admin-api.md) | 服务端 gRPC 管理接口（`messageloop.server.v2.APIService`）：鉴权与能力位、命名空间、8 个 RPC 的字段与语义、错误模型、grpcurl 示例、集群感知行为 |
