...
- **注册白名单**（`envConfigKeys`）：部署相关的标量键——`server.http.*`、`server.api.*`、`server.heartbeat.*`、`server.rpc_timeout`、`server.require_auth`、`server.namespace`、`server.limits.*`、四个传输的 addr/timeout/tls、`broker.*`、`cluster.*`。文件里省略这些键时，仅白名单键能凭环境变量出现。
- **文件内已有键**：`AutomaticEnv` 还会覆盖文件中已出现的任意标量键；字符串数组接受逗号分隔值。

复杂嵌套结构（`server.authorizer.rules`、`proxy` 后端数组）不支持环境变量，只能写配置文件。

### 改名迁移：Admin API → Server API

原「管理 API / Admin API」（`internal/admin`、`server.grpc_admin.*`）已整体更名为 **Server API（服务端 API）**——该接口定位是服务端集成面（发布/断连/代订/查询），不是管理后台。proto 线面 `messageloop.server.v2.APIService` 及 8 个 RPC 方法名**不变**，只改配置键、环境变量、指标名与代码内部命名。**不保留旧名兼容**，升级时按下表一次改齐：

| 旧 | 新 |
|---|---|
| `server.grpc_admin.addr` / `.tls.*` / `.auth_tokens` / `.allow_insecure` / `.capabilities` | `server.api.addr` / `.tls.*` / `.auth_tokens` / `.allow_insecure` / `.capabilities` |
| `server.grpc_admin.admin_auth_cache_ttl` | `server.api.auth_cache_ttl` |
| `proxy[].admin_auth: true` | `proxy[].api_auth: true` |
| `MESSAGELOOP_SERVER_GRPC_ADMIN_ADDR` / `_AUTH_TOKENS` / `_ADMIN_AUTH_CACHE_TTL` / `_ALLOW_INSECURE` / `_TLS_CERT_FILE` / `_TLS_KEY_FILE` | `MESSAGELOOP_SERVER_API_ADDR` / `_AUTH_TOKENS` / `_AUTH_CACHE_TTL` / `_ALLOW_INSECURE` / `_TLS_CERT_FILE` / `_TLS_KEY_FILE` |
| 指标 `messageloop_admin_rpc_total` / `messageloop_admin_auth_requests_total` / `messageloop_admin_user_fanout` | `messageloop_server_api_rpc_total` / `messageloop_server_api_auth_requests_total` / `messageloop_server_api_user_fanout` |
| proxy 契约 `ProxyService/AuthenticateAdmin`（`AdminIdentityInfo`） | `ProxyService/AuthenticateAPIKey`（`APIKeyInfo`）——mlbridge 需同步 |

不变项：静态超管 principal 字面量 `"admin"` 与 `key:<id>` 前缀（ACL allow 名单匹配语义不变）；运维 HTTP 面（`server.http.*`、realm `messageloop-admin`）；Torchwood whoami 契约。
...
5. **TLS 证书/密钥成对**：以下五处的 `cert_file` 与 `key_file` 必须同时设置或同时为空：`server.api.tls`、`transport.websocket.tls`、`transport.grpc.tls`、`transport.quic.tls`、`transport.kcp.tls`。
6. **Server API 鉴权**（`validateAPIAuth`）：`server.api.addr` 非空时必须至少配置一种凭证路径——`auth_tokens`、`proxy[].api_auth: true` 指派或 `allow_insecure: true`，否则 Validate 失败；每把 `auth_tokens` ≥ 20 字符；`auth_cache_ttl` 非空时必须是合法且为正的 Go duration；`proxy[].api_auth` 至多一个条目可指派且该条目必须带 `name`（G3）；`allow_insecure` 只允许搭配回环 `server.api.addr`（G5，非 loopback 即 Validate 错误）；`server.http.addr` 非 loopback 时必须配置 `server.http.auth_token`（admin HTTP 同规则对齐）。
...
9. **Server API 能力位闭集**：`server.api.capabilities` 的每个名字都必须在闭集内（见 [server 节](#server-节)）。
...
13. **Server API 地址必填**：`server.api.addr` 必填（`prepareGRPCServers` 无条件预绑定该监听器）。
...
```yaml
server:      # 服务端行为：管理 HTTP（运维）、Server API、心跳、限流、授权、命名空间
...
| `server.api.addr` | string | 未设置（**必填**） | Server API 监听地址。启动时无条件预绑定（见 [启动要求](#启动要求)）。接口清单见[《Server API 参考》](03-server-api.md) |
| `server.api.tls.cert_file` / `.key_file` | string | 未设置 | Server API 的 TLS 证书与私钥，二者必须成对设置（校验规则 5） |
| `server.api.auth_tokens` | string[] | 未设置 | 静态超管 Bearer token **列表**，通过 `authorization: Bearer <token>` 头传递，逐把常数时间比较，任一命中即超管身份（namespace `["*"]` + 能力位上限）。**每把 ≥ 20 字符**（Validate 强制）。列表形态让轮换无空窗：加新 → 滚动重启 → 删旧。**no-compat 替换**：单值键 `auth_token` 已删除，配置中出现会被忽略（三种凭证路径齐缺时 Validate 拒绝启动） |
| `server.api.auth_cache_ttl` | duration | `30s` | API Key 经 `api_auth` 指派 proxy 校验后的正缓存 TTL（`DefaultAuthCacheTTL`），同时是 Key 撤销传播的时间上界；生效值还会被 Key 自带 `max_age_seconds` 收紧、下限 1s。省略取默认；显式值必须为合法正 duration（Validate） |
| `server.api.allow_insecure` | bool | `false` | 显式放弃强制鉴权：无凭证请求注入 insecure 超管身份，每个请求记录 WARN。**仅限回环绑定**（G5 fail-closed）：非 loopback 的 `addr` 搭配 `true` 是 Validate 错误。仅限受控环境（开发/内网） |
| `server.api.capabilities` | string[] | 未设置 | Admin Capability 闭集：`history.read` / `presence.read` / `channels.list` / `session.act` / `user.fanout` / `subscribe.any` / `presence.large_snapshot` / `survey.bypass_gate` / `pattern.global`（预留）。**省略 = 除 `pattern.global` 外全部位**；**显式 `[]` = 零位**，锁死 Server API 数据面（GetHistory / GetPresence / GetChannels / 代订 / 按 user 扇出全部失败）；未知名 → Validate 错误 |
...
- **Survey**：默认拒绝；`Effects.Survey==true` 且 存在 `allow_survey` 命中该精确频道 且 无 deny 命中才放行。Server API 调用方无 `survey.bypass_gate` 能力位时同样受此名单约束。
- **恢复（Recover）/ 在场（Presence）**：精确频道；默认跟随 `Effects(ch)`；`deny_all` 命中或通配频道 → 拒绝。
- **Server API 调用方**：用户 ID 按 `"admin"` 匹配名单；另受 `server.api.capabilities` 能力位约束。
...
| `transient_only` | bool | `false` | 强制瞬时：发布只实时投递、绝不写历史。隐含 History=false、Recover=false（即使漏写）。对客户端：不带 `transient` 标志的发布也改走瞬时发布、ack offset=0、不报错；对 Server API：`add_history=true` 被拒绝（计失败、不发布），`add_history=false` 仍可瞬时发布 |
| `recover_limit` | int | 0 = `MaxRecoveredPublications` | 恢复条数上限。命中该上限（或请求级配额耗尽）时恢复结果标记 `truncated=true` |
| `max_survey_subscribers` | int | 256 | survey 订阅者上限。客户端 Survey 发起方本节点订阅者数（快路径，含通配命中）或集群预检总数超过该值 → `SURVEY_TOO_MANY_SUBSCRIBERS`、零条 outbound `SurveyRequest`。`0` = 不限制。Server API 调用方无 `survey.bypass_gate` 时同样受此门限制 |
...
| `presence_snapshot_limit` | int | 256 | `Connected.presence` / `SubscribeAck.presence` / `PresenceQuery` 快照的 clients 条数上限；`occupancy` 仍是全量计数，超出置 `truncated=true`。Server API `GetPresence` 无 `presence.large_snapshot` 能力位时同样截断到该上限 |

**transient_only 对客户端与 Server API 的差异**：
...
- **Server API Publish**：`add_history=true` 但策略禁历史 → 打 Warn、计失败、不发布（避免误以为写入了）；`add_history=false`（或缺省）仍走瞬时发布。若同请求其他发布成功，RPC 仍成功（部分成功语义），只有全部失败才返回错误。
...
`cmd/server/runtime.go` 在启动时无条件预绑定两个 gRPC 监听器（`transport.grpc.addr` 与 `server.api.addr`），任一失败（地址为空、端口被占用等）都会中止启动——与 `Validate()` 的必填校验一致。预绑定失败时先启动的监听器会被释放，不会泄漏端口。
...
proxy:
  - name: example-grpc          # 唯一标识
    ...
    # api_auth: true          # 指派为 Server API Key 校验器（全配置唯一，G3）
    grpc:                       # gRPC 后端配置（二选一）
      insecure: true
      # tls:
...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `name` | string | 未设置 | 代理标识，用于日志与排障；`api_auth: true` 时必填（按名解析校验器实例） |
| `endpoint` | string | 未设置 | 后端地址。gRPC 后端为 `host:port`；HTTP 后端为完整 URL（`http://` / `https://` 前缀） |
| `api_auth` | bool | `false` | 将该代理指派为**管理 API 的 API Key 校验器**（proxy 契约的 `AuthenticateAPIKey` RPC）。全配置至多一个条目可指派，两处指派即 Validate 错误（G3：显式布尔指派，不用方法名路由——**不存在 `$authenticate_admin`** 之类的管理面方法名）。指派与 `routes` 解耦：admin 认证不走 channel/method glob 匹配，客户端面 `$authenticate` 等钩子不受影响。指派语义与校验链详见[《Server API 参考》](03-server-api.md) |
...
| `AuthenticateAPIKey` | 管理 API 携带 API Key 凭证的一元请求 | **不走此路由表**：不按方法名/glob 匹配，唯一激活方式是 proxy 条目的 `api_auth: true` 显式指派（全配置唯一，见上表）；静态 `auth_tokens` 凭证不经过 proxy |
...
管理 gRPC 同样绑定回环地址。`addr` 必填，且非空时三种凭证路径（`auth_tokens`、`proxy[].api_auth: true` 指派、`allow_insecure`）必须至少配置一种：只写 `addr` 会直接启动失败。每把 token ≥ 20 字符（Validate）；列表形态支持轮换无空窗（加新 → 滚动重启 → 删旧）。注释掉的 `auth_cache_ttl` 是 API Key 正缓存 TTL（默认 30s，需要 `proxy[].api_auth` 指派才有意义）。注释掉的 `capabilities` 块展示了 Admin 能力位闭集——省略时除 `pattern.global` 外全位，显式 `[]` 锁死 Server API 数据面。需要 TLS 时取消 `tls` 段注释并成对填写证书与私钥。注意：这个地址（连同 `transport.grpc.addr`）在启动时都会被无条件预绑定，必须可监听。
...
- 各节点面向客户端的监听地址与 `server.http.addr` / `server.api.addr` 应各不相同（负载均衡器对外暴露，节点间不直接互连）；
