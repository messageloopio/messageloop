...
// rules, default Effects = DefaultChannelPolicy(). Rules whose pattern is not
...
func compileRule(rule config.AuthorizerRule) (compiledRule, error) {
	...
	compiled := compiledRule{
		...
		effects: compilePolicySpec(rule.ChannelPolicySpec),
	}
...
func (a *Authorizer) decideLocked(p Principal, action Action, channel string) Decision {
	...
	case ActionSurvey:
		...
		allowed := false
		surveyTouched := false
		if a.base.Survey != DefaultChannelPolicy().Survey {
			surveyTouched = true
		}
		for _, rule := range a.rules {
			if !topics.Match(rule.raw, channel) {
...
func (a *Authorizer) effectsLocked(channel string) ChannelPolicy {
	pol := a.base
	for _, rule := range a.rules {
		if topics.Match(rule.raw, channel) {
			pol = overlay(pol, rule.effects)
		}
	}
