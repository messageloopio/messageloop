...
func TestGRPC_ClientStream_PubSub(t *testing.T) {
	...
	server, err := grpc.PrepareClientServer(grpc.Options{
		Addr:         "127.0.0.1:0",
...
func TestGRPC_ClientStream_SubscribeUnsubscribe(t *testing.T) {
	...
	server, err := grpc.PrepareClientServer(grpc.Options{Addr: "127.0.0.1:0"}, node)
	require.NoError(t, err)
	startPreparedServer(t, server)
...
func TestGRPC_ClientStream_MultipleSubscribers(t *testing.T) {
	...
	streams := make([]googlegrpc.BidiStreamingClient[clientpb.InboundMessage, clientpb.OutboundMessage], numSubs)
	for i := 0; i < numSubs; i++ {
