...
func startTestQUICServer(t *testing.T, node *messageloop.Node) *quicstream.Server {
	t.Helper()
	server, err := quicstream.NewServer(quicstream.Options{
		Addr:         "127.0.0.1:0",
		Insecure:     true,
...
func dialQUIC(t *testing.T, addr string, alpn string) (*quic.Conn, *quic.Stream) {
	t.Helper()
	if alpn == "" {
...
func connectSession(t *testing.T, stream *quic.Stream, clientID string) *clientpb.Connected {
	t.Helper()
